...
{
    public class RemorseNailEX : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] == 0)
            {
                ...
                if (Projectile.Opacity >= 1f && player.controlUseItem)
                {
                    if (player.altFunctionUse == 2)
                    {
                        ...
                        Projectile.ai[1] = 0;
                        Projectile.ai[0] = 1;
                        if (target.X > Projectile.Center.X)
                            Projectile.spriteDirection = 1;
                    }
                }
                ...
                if (hasTarget)
                {
                    ...
                    {
                        if (p.type == ModContent.ProjectileType<RemorseHammerEX>() && p.owner == Projectile.owner)
                        {
            ...
            {
                Vector2 targetFlipped = target - FSkel.BoneName[Bone.ArmIK].GetParent.GetPosition(Projectile.spriteDirection);
                targetFlipped.X = Math.Abs(targetFlipped.X);
                ...
                else if (Projectile.ai[1] < time2)
                {
                    if (Projectile.ai[1] == time1)
                    {
                        SoundEngine.PlaySound(LobotomyCorp.ItemLobSound("GeneralWorks/Slientgirl_Volt"), Projectile.Center);
                    }

                    float time = (Projectile.ai[1] - time1) / time2;
                    time = 1 - (float)Math.Pow(1 - time, 3);

                    float targetRot = MathHelper.ToDegrees(targetFlipped.ToRotation());
                    ikDist = 0.6f + 0.3f * time;
                    ikRot = MathHelper.Lerp(-135, targetRot, time);
                    ikSpeed = -1;

                    handRot = ikRot;
                    handSpeed = -1;

                    shoulderRot = targetRot - 20;
                    shoulderSpeed = 0.7f;
                    Projectile.ai[0] = 2;
                }
                ...
                Vector2 targetPos = target - Projectile.Center.DirectionTo(target) * range;
                Vector2 vel = (targetPos - Projectile.Center) * 0.2f;
            ...
            else if (Projectile.ai[0] < 6)
            {
                ...
                else
                {
                    ...
                    {
                        float realTime = Projectile.ai[1] - timeStart;
                        float maxTime = 24;
                        int time1 = (int)(maxTime * 0.2f);
                        int time2 = time1 + time1;
                        int time3 = time2 + time1;
                        ...
                        else if (realTime < time2)
                        {
                            ...
                            shoulderRot = -120 + 80 * time;
                            Projectile.ai[0] = 5;
                        }
                        else if (realTime < time3 + time1)
                        {
                            ikDist = 1f;
                            ikRot = 10f;
                            ikSpeed = -1;
                        ...
                        if (Projectile.ai[1] >= 70 + maxTime * 4)
                        {
                            Projectile.ai[0] = 6;
                            Projectile.ai[1] = 0;
                        }
            ...
            Projectile.ai[1]++;
            FSkel.BoneName[Bone.Shoulder].ChangeRotation(MathHelper.ToRadians(shoulderRot + 5 * (float)Math.Sin(-Projectile.ai[1] * 0.017f)), shoulderSpeed);
            ...
            BonePart ik = FSkel.BoneName[Bone.ArmIK];
            ik.ChangeOffset(ikpos, ikSpeed);
            Vector2 min = ik.GetOffset;
            if (min.Length() < GetLength * 0.3f)
            {
                min.Normalize();
                ik.ChangeOffset(min * GetLength * 0.3f, -1);
            }
            FSkel.RotationIK(Bone.LowerArm, Bone.UpperArm, Bone.ArmIK);
        }

        private void SpawnSmear(Vector2 target, int time)
        {
            BonePart hand = FSkel.BoneName[Bone.Arm];
            Vector2 vel = hand.GetPosition().DirectionTo(target) * 16;
        ...
        private void InitializeSkeleton()
        {
            ...
            list.Add(Bone.Arm, new BonePart(Vector2.Zero, MathHelper.ToRadians(70), 1, 1, list[Bone.UpperArm])
                   .SetDraw(tex,
                   new Rectangle(0, 64, 224, 64),
                   new Vector2(32 * 3 + 16, 48)).
                    ChangeIRotation(false));
        ...
        public override void ModifyDamageHitbox(ref Rectangle hitbox)
        {
            if (FSkel != null)
            {
                ...
                armPos += new Vector2(75, -15 * Projectile.spriteDirection).RotatedBy(arm.GetRotation(Projectile.spriteDirection));
                int size = 90;
                int off = size / 2;
        ...
        {
            Main.player[Projectile.owner].GetModPlayer<LobotomyTethPlayer>().RemorseApplyNail(target.whoAmI, Projectile.ai[0] == 5);
        }
